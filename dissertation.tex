...
If we imagine Turing's infinitely long tape and the inertia that must be overcome to move such a tape left and right, it would not be too much of a leap of imagination to propose copying some sequential part of the tape onto a finite, lighter tape which could be moved left and right faster. And if the data required for the current part of our computation was contained within this faster tape, then we could conduct our computation faster, only refreshing the contents of the lighter tape as required. And if the speed at which we can perform operations on the lighter tape began to outpace the speed of movement of the tape, we might propose copying some of the data onto an even lighter, shorter, faster tape.
