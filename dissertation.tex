...
At the heart of every Raspberry Pi is a Broadcom "System on a Chip" (SoC) with Arm processor cores.
...
Initially released in 2012 as the Raspberry Pi 1, each model has seen improvements in SoC core count, clock speed and available memory.
...
The most recent addition to the range is the Raspberry Pi 4, sporting a quad-core 64-bit Cortex-A-72 based SoC clocked at 1.5 GHz. This model is available with 1, 2, 4 and 8 GB of RAM.

Since 2012 the official operating system for all Raspberry Pi models has been Raspbian, a Linux operating system based on Debian. To support the aims of the Foundation, a number of educational software packages are bundled with Raspbian, including a "non-commercial use" version of Mathematica, and a graphical programming environment aimed a younger children call Scratch.

Due to its popularity and ease of use, Python is the official programming language, and the inclusion of an easy to use Python IDE has been a Foundation priority. This is currently Thonny. 

Even though the Raspberry Pi 3 introduced a 64-bit processor, Raspbian has remained a 32-bit operating system. However, to complement the introduction of the Raspberry Pi 4 with 8 GB of RAM, a 64-bit version is currently in Beta testing.

Raspbian has recently been renamed Raspberry Pi OS.
