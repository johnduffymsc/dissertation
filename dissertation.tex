...
\lstset{style=hack}
\begin{lstlisting}
Pi Cluster Tools are not production quality, yet!
\end{lstlisting}
...
This is only achievable continuously if every instruction in a program is an FMA instruction, which obviously cannot be the case. Nevertheless, this is the standard measure of theoretical maximum performance.

The theoretical maximum performance of the Aerin Cluster is therefore:
...
For the High Performance Linpack benchmark, to achieve 100\% performance requires a problem size that utilises 100\% of memory. Because the operating system requires some memory, is it not possible to allocate 100\% memory for benchmarks.

The Linux command \verb|dmesg| prints out the kernel boot messages, which can be searched using \verb|grep| to determine how memory is utilised on the system:
...
Out of the 4 GB of memory per node, only 3.7 GB is available for user programs. 
...
As expected... no difference between a pure OpenMPI and hybrid OpenMPI/OpenMP for a single process...
