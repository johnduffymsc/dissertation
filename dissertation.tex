...
The HPL, HPCC and HPCG software packages were all compiled locally from source. The instructions for how to do this are in Part II Chapter 7.
...
\lstset{style=type}
\begin{lstlisting}
$ ~/picluster/tools/do "echo hello"
\end{lstlisting}


Probably the most useful of the Pi Cluster Tools are the two BLAS library tools.

\verb|libblas-query| queries the current ``BLAS library in use'' on each node and prints it out. This is extremely useful for ensuring the same library is in use on each node.
...
These two tools save a lot of typing, time and heartache.
