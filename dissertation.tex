...
It can be seen that 96.56\% of the time within the \verb|xhpl| process is spent in the \verb|HPL_dgemm| function, which subsequently calls the BLAS \verb|dgemm| function (the \verb|_| appended to \verb|dgemm| function name is a Fortran function name decoration).
...
where $A$ is a M x K matrix, $B$ is a K x N matrix, $C$ is a M x N matrix, and $\alpha$ and $\beta$ are scalars.
