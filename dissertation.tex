...
The block size NB is used for two purposes. Firstly, to ``block'' the problem size matrix of dimension N x N into sub-matrices of dimension NB x NB. This is described in more detail in the following section ?? BLAS Libraries. And secondly, as the message size (or multiples of) for distributing data between cluster nodes.
...
The HPL algorithm favours P x Q grids as square as possible, i.e. with P almost equal to Q, but with P smaller than Q. So, for a single node with 4 cores, a processor grids of 1 x 4 gives better benchmark performance than 2 x 2.
...
\subsection{Amend \texttt{macbook} \text{/etc/hosts}}
