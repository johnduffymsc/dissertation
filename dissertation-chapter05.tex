...
For 8 nodes, the highest $R_{max}$ of 103.41 Gflops was observed using the BLIS Serial library with an NB of 168, and a P x Q grid of 2 x 16. This is 67.32\% of the theoretical maximum performance using 80\% of memory.
...
However, the cluster topology does make a difference between the \emph{global} benchmarks. For the pure topology the update rate 0.00064 GUP/s for both random number algorithms. And, the hybrid topology update rate is 0.00047 GUP/s for both algorithms. The pure topology is 43\% faster for the same array size. This is likely to be due to the pure topology having more MPI processes to update the array.
...
The FFT benchmark measures the performance in computing the Discrete Fourier Transform of a large one-dimension vector.

The benchmark results are presented in Table 5.9.
...
The performance of the FFT benchmark is a small fraction of HPL performance. This is due to a number of factors. Double precision complex numbers occupy twice as much memory as real double precision numbers (16 bytes versus 8). The 128-bit Advanced SIMD registers are therefore only large enough to store one double precision complex number. And, the FFT algorithm involves exponential operations which are computationally expensive.

The \emph{global} performance is higher for the pure OpenMPI topology than the hybrid OpenMP/OpenMPI topology. This suggest the Cooley-Turkey algorithm is more efficient with a higher number of MPI processes.

The \emph{single} performance is higher than the \emph{star} performance. This is likely to be due to shared L2 cache access in the \emph{star} case.
...
The average OpenMP topology bandwidth is 112.682 MB/s, which is 90\% of the maximum theoretical bandwidth. The is consistent with 92.2\% of \emph{node-to-node} maximum theoretical bandwidth observed using the Linux \verb|iperf| command in Section 5.54.
...
In a similar manner to the TOP500 List, the HPCG List ranks computer systems in order of HPCG benchmark performance.

The June 2020 HPCG List ranks 169 computer systems.

Ranking number 1 in the list is the Fugaku supercomputer with a HPCG performance of 13.366 Pflops. 

Ranking 169 in the list is the Spaceborne Computer with a HPCG performance of 0.000034 Pflops.

The Spaceborne Computer is onboard the International Space Station (ISS). It is a 32 core system based on the Intel Xeon E5-2620 v4 8 core CPU, clocked at 2.1GHz, with an Infiniband interconnect.
...
The HPCG benchmark performance is approximately 20\% greater for a pure OpenMPI topology. This suggests it is more efficient to spread sparse data across a larger number of MPI processes. Since there is less opportunity to \emph{block} sparse data, and less reuse of data already resident in caches, a finer grained approach to data distribution and processing appears to be more efficient.  
...
\begin{enumerate}
\item The majority of the time in \emph{kernel space} is spent servicing software interrupts generated by network activity.
...
The results of running the \verb|linpack-profiler| tool with \emph{Adaptive RX} interrupt coalescing enabled are plotted in Figure 5.8.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{profile_interrupt_coalescing.pdf}
	\caption{\textbf{Adaptive RX interrupt coalescing.}}
	\label{fig:subim1}
\end{figure}

Using the value of the block size NB which achieved optimum performance running the \texttt{linpack-profiler} tool, the HPL benchmark results using 80\% of available memory are presented in Table 5.13. 
...
Although a 5.4\% improvement in HPL performance is observed for BLIS OpenMP, this improvement is not observed for the remaining three BLAS library combinations. A 3.5\% reduction in HPL performance is observed for the OpenBLAS OpenMP. And for the serial libraries, a 0.6\% increase in HPL performance is observed for OpenBLAS, and a 1.3\% reduction for BLIS.

It is therefore likely that the observed differences from baseline performance are within experimental error, and that enabling Adaptive RX interrupt coalescing has little or no effect.
...
The results of running the \verb|linpack-profiler| tool with \emph{Receive Packet Steering} and \emph{Receive Flow Steering} enabled are plotted in Figure 5.9.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\textwidth]{profile_rps_rfs.pdf}
	\caption{\textbf{Receive Packet Steering and Receive Flow Steering.}}
	\label{fig:subim1}
\end{figure}

Using the value of the block size NB which achieved optimum performance running the \texttt{linpack-profiler} tool, the HPL benchmark results using 80\% of available memory are presented in Table 5.14. 
...
Prior to enabling RPS, using the Linux command \verb|cat /proc/softriqs|, it was observed that \emph{CPU0} was the only core (on each node) to service the software interrupts generated by the receipt of network packets. After enabling RPS, using the same Linux command, it was observed that all cores now service the interrupts. RPS is therefore enabled and working. 
  
However, enabling RFS on top of RPS, has resulted in similar HPL benchmark results to the previous interrupt coalescing experiment. It is likely that the differences from baseline HPL performance are within experimental error.
...
The Pi Cluster Tools \verb|linpack-profiler| tool was run with a \emph{No Forced Preemption} kernel. The results are plotted in Figure 5.10.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{profile_nopreempt.pdf}
	\caption{\textbf{No Forced Preemption.}}
	\label{fig:subim1}
\end{figure}

Using the parameters which achieved optimum performance when running the \texttt{linpack-profiler} tool, the HPL benchmark results using 80\% of available memory are presented in Table 5.15. 
...
The output from the \verb|linpack-profiler| tool, as depicted in Figure 5.10, indicates that HPL performance of all BLAS libraries improves with a No Forced Preemption kernel. And, this is observed when running HPL using 80\% of memory for the BLIS OpenMP library, which achieves 106.94 Gflops. However, a reduction in HPL performance from baseline is observed for the remaining BLAS libraries.
...
The maximum observed baseline HPL performance was 103.41 Gflops.
