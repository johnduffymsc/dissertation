...
\hline
OpenBLAS OpenMP & 52416 &  72 & 2 &  4 &  &  \\
\hline
BLIS Serial     & 52320 & 120 & 2 & 16 &  & 1.0021e+02 \\
\hline
BLIS OpenMP     & 52320 & 160 & 2 &  4 &  & 1.0584e+02 \\
\hline
\end{tabular}
\end{center}
...
As discussed in Chapter 2, each packet received by a network interface generates a hardware interrupt. This results in a context switch to the kernel to process the packet. \emph{Interrupt Coalescing} delays the raising of a hardware interrupt until a number of packets have been received, therefore reducing the number of context switches. This potentially improves throughput and benchmark performance. 

The \verb|linpack-profiler| tool was run with \emph{Interrupt Coalescing} enabled using the \verb|interrupt-coalescing| tool. Both tools are part of Pi Cluster Tools. The results are plotted in Figure ??.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{profile_nopreempt_8_node_20_percent_memory.pdf}
	\caption{Pi Cluster Tools \texttt{linpack-profiler} $R_{max}$ with \emph{Interrupt Coalescing} enabled.}
	\label{fig:subim1}
\end{figure}
...
\end{tabular}
\end{center}
\caption{\label{tab:table-name}Comparison of the Baseline HPL $R_{max}$ with \emph{Interrupt Coalescing} enabled.}
\end{table}
